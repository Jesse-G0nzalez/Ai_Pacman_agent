...
SAVE_PATH = "./models/double_dqn_mspacman.pth"
LOG_PATH = "./logs/training_log_more_metrics.csv"
...
def select_actions(states, steps_done, policy_net, n_actions, device):
    ...
    exploration_rate = num_random / len(states)  
    return actions, eps, exploration_rate
...
def main():
    ...
    steps_done = 0
    last_loss = None
    rewards_buffer = deque(maxlen=100)  # for moving average reward
    ...
    # CSV log setup with new columns
    ...
    try:
        while True:
            # Select actions with metrics
            actions, eps, exploration_rate = select_actions(
                states, steps_done, policy_net, n_actions, device
            )
            ...
            # Store transitions and update episode lengths
            ...
            if len(memory) >= BATCH_SIZE:
                ...
                with torch.no_grad():
                    ...
                    next_q_vals  = target_net(s2).gather(1, next_actions).squeeze()
            ...
            # Compute mean Q-value for current state batch
            with torch.no_grad():
                qvals = policy_net(states).max(dim=1)[0]
                mean_q = qvals.mean().item()
            ...
            for i in range(NUM_ENVS):
                if dones[i]:
                    episode += 1
                    moving_avg = np.mean(rewards_buffer) if rewards_buffer else 0.0 
                    ...
                    print(f"Episode {episode} | Reward {total_rewards[i]:.1f} | "
                          f"MovAvg {moving_avg:.1f} | Len {length} | "
                          f"Loss {last_loss:.4f} | MeanQ {mean_q:.2f} | "
                          f"Eps {eps:.3f} | ExpRate {exploration_rate:.2f}")

                    rewards_buffer.append(total_rewards[i])
                    total_rewards[i] = 0.0
                    episode_lengths[i] = 0  # reset for next episode
